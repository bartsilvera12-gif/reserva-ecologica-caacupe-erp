...
/** GET /api/reportes/ventas?mes=YYYY-MM */
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getReporteVentas(schema, ctx.auth.empresa_id, exigirSucursal(ctx.auth.sucursal_id), { mes, start, end, mesInicio: `${mes}-01` });
