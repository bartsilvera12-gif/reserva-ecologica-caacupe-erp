/**
 * Migración: completar flujo sorteo_default después de cédula.
 */
...
async function main() {
  const sql = readFileSync(
    join(process.cwd(), "supabase", "migrations", "20250329190000_chat_flow_sorteo_default_extend.sql"),
    "utf-8"
  );
  const client = new Client({ connectionString: getDbUrl(), ssl: { rejectUnauthorized: false } });
  try {
    await client.connect();
    console.log("Ejecutando migración extensión sorteo_default...");
